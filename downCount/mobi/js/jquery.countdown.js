...
jQuery.fn.countdown = function(userOptions)
{
  var options = {
    ...
    digitImages: 6,
    digitWidth: (440/750*67),
    digitHeight: (440/750*90),
    timerEnd: function(){},
    image: "/js/digits.png"
  };
    ...
    for (var i = 0; i < options.startTime.length; i++)
    {
      if (parseInt(options.startTime[i]) >= 0)
      {
        elem = $('<div id="cnt_' + c + '" class="cntDigit" />').css({
          height: (options.digitHeight/100 + 'rem'),
          float: 'left',
          background: 'url(\'' + options.image + '\') center top',
          'background-size': (440/750*67/100) + 'rem ' + (49.5*440/750) +'rem',
          width: (options.digitWidth/100 + 'rem')
        });

        elem.current = parseInt(options.startTime[i]);
        digits.push(elem);
		console.log(elem.css('background-posit	ion'));
  ...
  var margin = function(elem, val)
  {
    if (val !== undefined)
    {
      ...
      return digits[elem].css({'backgroundPosition': '0 ' + (val/100) + 'rem'});
    }
